/**
 * MilletKurzgesagt.tsx — "Hạt kê" explainer in the Kurzgesagt-style flat-vector
 * language. Built with .claude/skills/kurzgesagt-style-video.
 *
 * Narration is the five existing Vietnamese TTS tracks in public/audio/ (the same
 * millet script the flat-explainer composition uses), and every visual is drawn in
 * code — this video pulls in no images and no stock footage at all.
 *
 * Scene durations come from the real audio lengths measured with
 * @remotion/media-parser, so the captions stay in sync with the voiceover:
 *
 *   scene-01-survival-seeds    18.08s -> 551 frames  (split across 2 visual beats)
 *   scene-02-growth-chart      13.19s -> 405 frames
 *   scene-03-farming-migration 11.68s -> 359 frames
 *   scene-04-corridor-map      13.43s -> 412 frames
 *   scene-05-scale-value        9.95s -> 308 frames
 */
...
export const MILLET_CANVAS = { width: 1080, height: 1920, fps: 30 } as const;
...
/**
 * Visual beats. Note beat 1 and 2 share ONE narration track: the opening line runs
 * 18s, which is too long to hold a single image, so the voiceover plays across two
 * beats while the visuals change underneath it.
 */
const SCENES: SceneSpec[] = [
  {
    ...
    caption:
      "Một nắm hạt nhỏ bé này từng là ranh giới giữa sự sống và cái chết…",
  ...
  },
  {
    ...
    archetype: "compare",
    caption:
      "Lúa nước cần tới 150 ngày để thu hoạch — trong khi giống lúa nhanh nhất chỉ mất 45 ngày.",
    durationInFrames: 405,
    audio: "audio/scene-02-growth-chart.mp3",
  ...
  },
  {
    ...
    archetype: "crowd",
    caption:
      "Người du mục vừa gieo hạt, vừa tiếp tục di chuyển không ngừng qua các thảo nguyên.",
    durationInFrames: 359,
    audio: "audio/scene-03-farming-migration.mp3",
    accent: MINT,
  },
  {
    id: "corridor",
    ...
    archetype: "flowMap",
    caption:
      "Từ Trung Á, hạt kê theo chân họ lan tới tận Lưỡng Hà — hàng ngàn cây số xa xôi.",
    durationInFrames: 412,
    audio: "audio/scene-04-corridor-map.mp3",
    accent: PINK,
  ...
  },
  {
    ...
    mood: "space",
    archetype: "cutaway",
    caption: "Với họ, một nắm hạt giống đôi khi còn quý hơn cả thanh kiếm.",
    durationInFrames: 308,
    audio: "audio/scene-05-scale-value.mp3",
    accent: VIOLET,
    rings: [
...
export const MILLET_TOTAL_FRAMES = SCENES.reduce((t, s) => t + s.durationInFrames, 0);
...
export const MilletKurzgesagt: React.FC = () => {
  let acc = 0;
